...
# 不写死任何单位内网代理：需要时传 --proxy http://host:port
DEFAULT_PROXY = ""
...
def push(token: str, owner: str, repo: str, branch: str) -> None:
    ...
    try:
        # ⚠ 走代理推大仓库会随机 reset（curl 55 Send failure）。
        #   必须：关压缩 + 锁 HTTP/1.1 + 调大 postBuffer + 失败重试。
        #   实测 30MB 仓库：不开这些必挂，开了 72s 一次过。
        last_err = ""
        for i in range(1, 4):
            r = git("-c", "core.compression=0", "-c", "http.version=HTTP/1.1",
                    "-c", "http.postBuffer=524288000", "-c", "http.lowSpeedLimit=1",
                    "-c", "http.lowSpeedTime=600",
                    "push", "-q", with_token, f"{branch}:{branch}", check=False)
            if r.returncode == 0:
                say(OK, f"已推送 {branch} → {safe}" + (f"（第 {i} 次尝试）" if i > 1 else ""))
                break
            last_err = _mask((r.stderr or "") + (r.stdout or "")).strip()[-200:]
            say(WARN, f"第 {i} 次推送失败：{last_err}")
            time.sleep(10)
        else:
            raise SystemExit(f"{FAIL} 推送失败（已重试 3 次）：{last_err}")
    finally:
...
def upload_asset(token: str, full: str, rel: dict, path: Path,
    ...
    say(FAIL, f"{tries} 次都没传上去。可手动兜底（需要代理就加 -x http://host:port）：\n"
              f"   curl -X POST --data-binary @{path} \\\n"
